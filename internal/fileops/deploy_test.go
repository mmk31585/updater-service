...
func TestDeployerBackupTargetNotFound(t *testing.T) {
	...
	d := NewDeployer()
	err := d.Backup(filepath.Join(tmpDir, "nonexistent.txt"), backup)
	if err == nil {
		t.Error("Backup() expected error for nonexistent target, got nil")
	}
}
...
func TestDeployerBackupAndApply(t *testing.T) {
	...
	if err := d.Backup(source, backup); err != nil {
